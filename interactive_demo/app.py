...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, model):
        ...
        # undo
        master.bind('q', lambda event: self.controller.undo_click())
        # finish annotation, save and next frame
        master.bind('e', lambda event: self.controller.partially_finish_object())
    ...
    def _add_buttons(self):
        ...
        self.next_clicks_button = \
            FocusButton(self.clicks_options_frame, text='Next Frame', bg='#ea9999', fg='black', width=10, height=2,
                        state=tk.DISABLED, command=self._reset_last_object)
        self.next_clicks_button.pack(side=tk.LEFT, fill=tk.X, padx=10, pady=3)
