...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas = canvas
        self.canvas.configure(xscrollcommand=self.hbar.set, yscrollcommand=self.vbar.set, cursor="tcross")
    ...
    def __size_changed(self):
        ...
        #print("canvas size is changed")
        self.__show_image()

    def set_focus(self, scale, center):
        center_y, center_x = center

        box_image = self.canvas.coords(self.container)
        center_x = center_x * self.real_scale[0] + box_image[0]
        center_y = center_y * self.real_scale[1] + box_image[1]
        x = self.canvas.canvasx(center_x)  # get coordinates of the event on the canvas
        y = self.canvas.canvasy(center_y)
        
        if self.outside(x, y): return  # zoom only inside image area
        scale *= self.__delta
        #print(center_x, center_y, x, y, self.current_scale)
    ...
    def __wheel(self, event):
        ...
        #print("event coor", event.x, event.y)
        x = self.canvas.canvasx(event.x)  # get coordinates of the event on the canvas
        y = self.canvas.canvasy(event.y)
        if self.outside(x, y): return  # zoom only inside image area
